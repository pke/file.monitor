...
public interface Kernel32 extends W32API {
	...
	HANDLE OpenProcess(int dwDesiredAccess, boolean bInheritHandle,
			int dwProcessId);

	boolean ReadProcessMemory(HANDLE hProcess, Pointer lpBaseAddress,
			Memory lpBuffer, SIZE_T nSize, IntByReference dwNumberOfBytesRead);

	boolean ReadProcessMemory(HANDLE hProcess, Pointer lpBaseAddress,
			Pointer lpBuffer, SIZE_T nSize, IntByReference dwNumberOfBytesRead);

	boolean ReadProcessMemory(HANDLE hProcess, int lpBaseAddress,
			Pointer lpBuffer, SIZE_T nSize, IntByReference dwNumberOfBytesRead);

	boolean ReadProcessMemory(HANDLE hProcess, int lpBaseAddress,
			byte[] lpBuffer, SIZE_T nSize, IntByReference dwNumberOfBytesRead);

	boolean ReadProcessMemory(HANDLE hProcess, int lpBaseAddress,
			ByReference lpBuffer, SIZE_T nSize,
			IntByReference dwNumberOfBytesRead);

	boolean WriteProcessMemory(HANDLE hProcess, Pointer lpBaseAddress,
			Pointer lpBuffer, SIZE_T nSize,
			IntByReference lpNumberOfBytesWritten);

	boolean WriteProcessMemory(HANDLE hProcess, int lpBaseAddress,
			Pointer lpBuffer, SIZE_T nSize,
			IntByReference lpNumberOfBytesWritten);

	int MEM_COMMIT = 0x1000;
	int MEM_RESERVE = 0x2000;
	int FILE_MAP_WRITE = SECTION_MAP_WRITE;
	int FILE_MAP_READ = SECTION_MAP_READ;
	...
	boolean VirtualFreeEx(HANDLE hProcess, Pointer lpAddress, SIZE_T dwSize,
			int dwFreeType);

	boolean VirtualFreeEx(HANDLE hProcess, int lpAddress, SIZE_T dwSize,
			int dwFreeType);

	boolean VirtualFreeEx(HANDLE hProcess, int lpAddress, int dwSize,
			int dwFreeType);
	...
	int TH32CS_SNAPALL = TH32CS_SNAPHEAPLIST | TH32CS_SNAPMODULE
	...
	boolean Module32First(HANDLE hSnapshot, MODULEENTRY32W lpme);

	boolean Module32Next(HANDLE hSnapshot, MODULEENTRY32W lpme);
